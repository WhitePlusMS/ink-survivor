...
## v3.0 (2026-02-13) - 前端页面编译错误修复

### 修改内容

#### 修复首页 heat 值获取
- **文件**: `src/app/page.tsx`
- **问题**: 使用了已废弃的 `b.heat` 字段
- **修复**: 改为 `b.score?.heatValue ?? 0` 从 Score 表获取热度值
- **修改内容**:
  - 排序逻辑: `b.heat - a.heat` → `(b.score?.heatValue ?? 0) - (a.score?.heatValue ?? 0)`
  - 日志输出: 改为输出 `heatValue`
  - 书籍映射: `heat: b.score?.heatValue ?? 0`

#### 修复书籍详情页 heat 和 chapterCount
- **文件**: `src/app/book/[id]/page.tsx`
- **问题**: 使用了已废弃的 `book.heat` 和 `book.chapterCount` 字段
- **修复**:
  - 从 `book.score?.heatValue` 获取热度
  - 从 `book._count?.chapters` 获取章节数
- **同步修改**: `src/services/book.service.ts`
  - 在 `getBookById` 方法中添加 `_count: { select: { chapters: true, comments: true } }`

#### 修复个人资料页统计字段
- **文件**: `src/app/profile/page.tsx`
- **问题**: 使用了已废弃的 `user.booksWritten` 和 `user.highestRank` 字段
- **修复**:
  - `booksWritten`: 从 `level?.booksWritten ?? 0` 获取（UserLevel 表）
  - `highestRank`: 从参赛记录 `participations` 中动态计算最小排名值
  - `book.chapterCount`: 改为 `book._count?.chapters ?? 0`

### 修改文件清单
| 文件 | 操作 |
|------|------|
| `src/app/page.tsx` | 修改 |
| `src/app/book/[id]/page.tsx` | 修改 |
| `src/app/profile/page.tsx` | 修改 |
| `src/services/book.service.ts` | 修改 |
